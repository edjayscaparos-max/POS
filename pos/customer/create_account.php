...
if (isset($_POST['addCustomer'])) {
    ...
    if (empty($_POST["customer_phoneno"]) || empty($_POST["customer_name"]) || empty($_POST['customer_email']) || empty($_POST['customer_password'])) {
        ...
        $customer_password = $_POST['customer_password']; // Get raw password for validation
        $customer_id = $_POST['customer_id'];

        // Validate password strength
        if (!preg_match('/[A-Z]/', $customer_password) || // At least one uppercase letter
            !preg_match('/[0-9]/', $customer_password) || // At least one number
            !preg_match('/[\W_]/', $customer_password)    // At least one special character
        ) {
            $err = "Password must include at least one capital letter, one number, and one special character.";
        } else {
            // Hash the password for security
            $customer_password = sha1(md5($customer_password)); // Double encrypt for security
